...
def zemin_tork_katsayisi(zemin_tipi: str) -> float:
    mapping = {
        "Dolgu": 40, "Kil": 60, "Silt": 55, "Kum": 70,
        "Çakıl": 90, "Ayrışmış Kaya": 130, "Kumtaşı": 150,
        "Kireçtaşı": 165, "Sert Kaya": 190,
    }
...
def gerekli_tork_hesapla(df: pd.DataFrame, cap_mm: float) -> float:
    """
    Fore kazık tork gereksinimi.
    Formül: T = tau * pi * D^3 / 8 * SF
    tau: zemin kayma direnci (kPa)
      - Kil: Su = SPT * 4 kPa (Terzaghi & Peck)
      - Kum: tau = SPT * 2 kPa (FHWA)
      - Kaya: tau = UCS(kPa) / 10 (ISRM)
    SF = 1.25 (saha güvenlik faktörü)
    Referans: FHWA-NHI-10-016, Bauer teknik kataloğu
    """
    cap_m = cap_mm / 1000.0
    max_tork = 0.0
    ...
    for _, row in df.iterrows():
        ...
        if ucs > 0:
            ...
            # Kil/silt: undrained shear strength
            tau = max(spt * 4.0, 20.0)
        else:
            # Kohezyonsuz: sürtünme direnci
            tau = max(spt * 2.0, 15.0)
        ...
        # tau (kPa = kN/m²) * m³ = kNm
        sf = 1.25
        katman_tork = tau * math.pi * (cap_m ** 3) / 8.0 * sf
...
def rop_hesapla(zemin_tipi: str, ucs: float, cap_mm: float) -> float:
    cap_m = cap_mm / 1000.0
    baz_rop = {
        "Dolgu": 8.0, "Kil": 6.0, "Silt": 6.5, "Kum": 5.0,
        "Çakıl": 3.5, "Ayrışmış Kaya": 2.0, "Kumtaşı": 1.2,
        "Kireçtaşı": 0.9, "Sert Kaya": 0.5,
    }.get(zemin_tipi, 3.0)
...
def tahmini_kazik_suresi(df: pd.DataFrame, cap_mm: float, kazik_boyu: float, casing_m: float) -> float:
    ...
    cap_m = cap_mm / 1000.0
    hacim = math.pi * (cap_m / 2) ** 2 * kazik_boyu
    sure += hacim * (20 / 60)
...
def makina_uygunluk(row, gerekli_tork, kazik_boyu, kazik_capi, casing_gerekli):
    ...
    if tork < gerekli_tork * 0.80:
        return "Uygun Değil", f"Tork yetersiz ({tork} / {gerekli_tork} kNm)"
